...
def get_args_parser():
    ...
    parser.add_argument('--data-set', default='IMNET', 
                        choices=['CIFAR', 'IMNET', 'INAT', 'INAT19', 'FLOWERS', 'PETS', 'AIRCRAFT', 'CARS', 'CALTECH101', 'CUSTOM'],
...
def main(args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if (epoch + 1) % args.test_eval_epochs == 0 or (epoch == args.epochs - 1):
            ...
            # Log with test stats included
            log_stats = {**{f'train_{k}': v for k, v in train_stats.items()},
                         **{f'test_{k}': v for k, v in test_stats.items()},
                         'epoch': epoch,
                         'n_parameters': n_parameters}
        else:
            # On epochs without testing, only log the train stats
            log_stats = {**{f'train_{k}': v for k, v in train_stats.items()},
                         'epoch': epoch,
                         'n_parameters': n_parameters}
